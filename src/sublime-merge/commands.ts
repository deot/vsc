...
export class RegisterCommands {
	...
	init() {
		let openInSublimeMerge = vscode.commands.registerCommand('deotsm.openInSublimeMerge', () => {
			this._runSublimeMerge(['.']);
		});

		let blameInSublimeMerge = vscode.commands.registerCommand('deotsm.blameInSublimeMerge', () => {
		...
		let fileHistoryInSublimeMerge = vscode.commands.registerCommand('deotsm.fileHistoryInSublimeMerge', () => {
			this._runSublimeMerge(['search', `file:"${this._currentFileRelativePathToRepo()}"`]);
		});

		let lineHistoryInSublimeMerge = vscode.commands.registerCommand('deotsm.lineHistoryInSublimeMerge', () => {
			const editor = vscode.window.activeTextEditor;
			if (editor) {
				const selectionInfo = editor.selection;
				const searchQuery =
		...
		let myCommitsInSublimeMerge = vscode.commands.registerCommand('deotsm.myCommitsInSublimeMerge', () => {
	...
	private _runSublimeMerge(args: string[]) {
		...
		proc.on('error', async err => {
			...
			const openSettings = await vscode.window.showWarningMessage(
				...
				'Set or check "deotsm.smergeExecutablePath"'
			);
			if (openSettings) {
